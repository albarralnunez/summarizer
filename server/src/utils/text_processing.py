...
def tokenize_and_clean_sentence(
    sentence: str, language: LanguageType = "english"
) -> List[str]:
...
def calculate_text_statistics(
    sentences: List[str], language: LanguageType = "english"
) -> TextStatistics:
    ...
    return TextStatistics(
        ...
        avg_word_length=(
            round(total_word_length / total_words, 2) if total_words > 0 else 0
        ),
        avg_sentence_length=(
            round(total_words / sentence_count, 2) if sentence_count > 0 else 0
        ),
