...
def calculate_text_statistics(
    processed_data: ProcessedData, word_scores: Optional[Dict[str, float]] = None
) -> TextStatistics:
    ...
    most_common = sorted(content_words.items(), key=lambda x: (-x[1], x[0]))[:10]

    # Process top scoring words if available
    top_scoring_words = []
    if word_scores:
        filtered_scores = {
            word: score for word, score in word_scores.items() if word not in stop_words
        }
...
async def summarize_text(
    ...
    if algorithm == "sklearn":
        ...
        summary = summarize_sklearn(sentences, num_sentences, language)
...
def summarize_simple(
    ...
    for sentence in processed_data.sentences:
        ...
        filtered_words = [
            ...
            if (word in processed_data.word_to_index and word not in stop_words)
        ]
        ...
        sentence_vectors.append(
            csr_matrix((data, (row, col)), shape=(1, len(processed_data.word_to_index)))
        )
    ...
    if compute_statistics:
        ...
        if word_scores:
            ...
            if max_score > 0:
                word_scores = {
                    word: score / max_score for word, score in word_scores.items()
                }
...
def summarize_sklearn(
    sentences: List[str], num_sentences: int, language: LanguageType
) -> List[str]:
...
async def summarize_default(
    ...
    if compute_statistics:
        ...
        word_scores = {
            ...
            for word, idx in processed_data.word_to_index.items()
            if (
                word not in STOP_WORDS[processed_data.language]
                and len(word) > 5
                and word_importance[idx] > 0
                and word.isalnum()
            )
...
def process_sentence(
    sentence: TokenizedSentence, word_to_index: Dict[str, int], language: LanguageType
) -> csr_matrix:
...
def process_chunk_vectorize(
    chunk_data: Tuple[List[str], Dict[str, int], LanguageType]
) -> List[csr_matrix]:
...
async def vectorize_sentences(
    word_to_index: Dict[str, int], sentences: List[str], language: LanguageType
) -> List[csr_matrix]:
