...
class TextVectorizer:
    ...
    def vectorize_sklearn(sentences: List[str], language: LanguageType) -> csr_matrix:
        ...
        processed_sentences = [
            ' '.join([
                word for word in word_tokenize(sentence.lower(), language)
                if (word not in STOP_WORDS[language] and 
                    word.isalnum() and 
                    len(word) > 2)
            ])
    ...
    @staticmethod
    def _process_sentence(sentence: str, word_to_index: Dict[str, int], language: LanguageType) -> csr_matrix:
    ...
    def _process_chunk_vectorize(cls, chunk_data: tuple) -> List[csr_matrix]:
        ...
        return [cls._process_sentence(sent, word_to_index, language) for sent in sentences] 