...
class TextProcessor(BaseTextProcessor):
    ...
    def _tokenize_sentence(self, sentence: str) -> TokenizedSentence:
        ...
        tokens = [
            word for word in self.tokenize_sentence(sentence)
    ...
    def _calculate_early_termination(
        ...
        return min(
            max(base_sentences, min_sentences),
            total_sentences
        )
    ...
    def _is_valid_sentence(self, sentence: str, min_length: int = 40) -> bool:
        ...
        if (len(cleaned) < min_length or  # Too short
            not any(c.isalpha() for c in cleaned) or  # No letters
            cleaned.count(' ') < 3):  # Too few words
            return False
            
        # Check for common invalid patterns
        invalid_patterns = [
            'figure', 'fig.', 'table', 'chapter',  # References
            'http://', 'https://', 'www.',  # URLs
            '@', '#',  # Social media
        ]
    ...
    async def process_text(
        ...
        if not sentences:
            return ProcessedData(
                word_to_index={},
                sentences=[],
                metrics=TextMetrics(0, 0, 0, 0, Counter()) if compute_statistics else None,
                language=self.language
            )
        
        # Filter and tokenize sentences in one pass
        valid_sentences = [
            self._tokenize_sentence(s) for s in sentences 
        ...
        if not compute_statistics and num_sentences:
            try:
                ...
                logger.warning(f"Failed to apply early termination: {e}. Processing all sentences.")
                sentences_to_process = valid_sentences
        
        # Process chunks with tokenized sentences
        chunk_size = max(1000, len(sentences_to_process) // (multiprocessing.cpu_count() * 2))
        chunks = [
    ...
    def _merge_chunk_results(
        ...
        for local_word_to_index, local_counts, local_total, local_length in chunk_results:
