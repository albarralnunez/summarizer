...
from src.services.dask_summarizer import summarize_text_dask, EmptyInputError, ProcessingError
...
def get_function_profiles(ps):
    ...
    for (filename, line_no, func_name), (calls, _, total_time, cumulative_time, _) in ps.stats.items():
        if 'src/services' in str(filename) and 'wrapper' not in str(func_name):
            ...
            function_profiles.append(
                FunctionProfile(
                    ...
                    percentage=(total_exec_time/total_service_time) * 100 if total_service_time > 0 else 0,
                    calls=calls,
                    memory_per_call=0
                )
            )
...
async def health_check(settings: DaskSettings = Depends(get_dask_settings)):
    ...
    health_status = {"status": "healthy", "api": "healthy", "dask": {"status": "healthy"}}

    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"http://{settings.scheduler_host}:{settings.dashboard_port}/health") as response:
        ...
        health_status["dask"].update({
            "workers": workers,
            "scheduler": settings.scheduler_address
        })
